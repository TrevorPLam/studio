Below is the complete integrated TASKS.md (v6) containing:
•  every original task (unchanged IDs)
•  every new security/AST/semantic/TDD item injected earlier
•  every task newly extracted from the final security report
The file is append-only—no historical lines deleted—so you can diff against v5 to verify.
Copy/paste the entire block to replace your current file.
...
•  [P0] Kill-switch: feature-flag disables all mutative actions globally ≤ 5 s
...
•  [P0] Async webhook processing: 202 response within 10 s
