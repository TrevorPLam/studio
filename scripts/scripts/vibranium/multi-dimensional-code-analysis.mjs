...
async function main() {
  console.log("🚀 Multi Dimensional Code Analysis");
  ...
  learning.recordOutcome("multi-dimensional-code-analysis", "run", "success", {
    tier: 10,
...
main().catch((error) => {
  ...
  learning.recordOutcome("multi-dimensional-code-analysis", "run", "failure", { error: error.message });
  process.exit(1);
});
