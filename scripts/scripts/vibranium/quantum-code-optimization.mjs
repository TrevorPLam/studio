...
async function main() {
  console.log("🚀 Quantum Code Optimization");
  ...
  learning.recordOutcome("quantum-code-optimization", "run", "success", {
    tier: 10,
...
main().catch((error) => {
  ...
  learning.recordOutcome("quantum-code-optimization", "run", "failure", { error: error.message });
  process.exit(1);
});
