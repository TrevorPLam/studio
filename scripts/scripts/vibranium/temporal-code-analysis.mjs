...
async function main() {
  console.log("🚀 Temporal Code Analysis");
  ...
  learning.recordOutcome("temporal-code-analysis", "run", "success", {
    tier: 10,
...
main().catch((error) => {
  ...
  learning.recordOutcome("temporal-code-analysis", "run", "failure", { error: error.message });
  process.exit(1);
});
