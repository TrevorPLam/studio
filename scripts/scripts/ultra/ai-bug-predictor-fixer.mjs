...
async function main() {
  console.log("🚀 Ai Bug Predictor Fixer");
  ...
  learning.recordOutcome("ai-bug-predictor-fixer", "run", "success", {
    tier: 9,
...
main().catch((error) => {
  ...
  learning.recordOutcome("ai-bug-predictor-fixer", "run", "failure", { error: error.message });
  process.exit(1);
});
