...
async function main() {
  console.log("🚀 Natural Language To Code");
  ...
  learning.recordOutcome("natural-language-to-code", "run", "success", {
    tier: 9,
...
main().catch((error) => {
  ...
  learning.recordOutcome("natural-language-to-code", "run", "failure", { error: error.message });
  process.exit(1);
});
